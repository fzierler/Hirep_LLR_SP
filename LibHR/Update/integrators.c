...
static suNg_av_field *tmp_momenta_llr = NULL;


void gforce(suNg_av_field *force, double dt, int nmon, int level, const monomial **mon_list)
{
	double S_llr=0;
	int used_llr = 0;

	if (tmp_momenta_llr == NULL){
		tmp_momenta_llr = alloc_avfield(&glattice);
	}
	...
	_MASTER_FOR(&glattice, ix)
	{
		...
		{
			_algebra_vector_zero_g(tmp_momenta_llr->ptr[4 * ix + dx]);
		}
	}
	...
	for (int n = 0; n < nmon; n++)
	{
		...
		{
			m->force_and_action_f(dt, tmp_momenta_llr, m->force_par, &S_llr);
			used_llr = 1;
			lprintf("Action from force", 0, "level %d mon %d force: %lf \n",level, m->data.type, S_llr);
		}
		else
	...
	if (used_llr == 1)
	{
		lprintf("LLR", 0, "Rescaling factor: %lf \n", (get_llr_a() + (S_llr - getS0()) / (getdS() * getdS())));

		_MASTER_FOR(&glattice, ix)
		{
			for (int dx = 0; dx < 4; dx++)
			{
				_algebra_vector_mul_add_assign_g(force->ptr[4 * ix + dx],
												 (get_llr_a() + (S_llr - getS0()) / (getdS() * getdS())),
												 tmp_momenta_llr->ptr[4 * ix + dx]);
			}
		}
	}
}
...
void leapfrog_multistep(suNg_av_field *momenta, double tlen, integrator_par *int_par)
{
	...
	for (int n = 1; n < int_par->nsteps; n++)
	{
		...
		lprintf("MD_INT", 20, "Inside integration loop step=%d", n);
		gforce(momenta, dt, int_par->nmon, int_par->level,int_par->mon_list);
		if (ipn)
		{
...
void O2MN_multistep(suNg_av_field *momenta, double tlen, integrator_par *int_par)
{
	...
	integrator_par *ipn = int_par->next;
	gforce(momenta, lambda * dt, int_par->nmon, int_par->level, int_par->mon_list);
	...
	for (int n = 1; n <= int_par->nsteps; n++)
	{
		if (ipn)
		{
			...
			ipn->integrator(momenta, dt / 2., ipn);
			gforce(momenta, (1. - 2. * lambda) * dt, int_par->nmon, int_par->level, int_par->mon_list);
			ipn->integrator(momenta, dt / 2., ipn);
		}
		else
		{
		...
		if (n < int_par->nsteps)
		{
			gforce(momenta, 2. * lambda * dt, int_par->nmon, int_par->level, int_par->mon_list);
...
void O4MN_multistep(suNg_av_field *momenta, double tlen, integrator_par *int_par)
{
	...
	integrator_par *ipn = int_par->next;
	gforce(momenta, r1 * dt, int_par->nmon, int_par->level, int_par->mon_list);
	...
	for (int n = 1; n <= int_par->nsteps; n++)
	{
		...
		if (ipn)
		{
			...
			gforce(momenta, r3 * dt, int_par->nmon, int_par->level, int_par->mon_list);
			ipn->integrator(momenta, r4 * dt, ipn);
			gforce(momenta, (0.5 - r1 - r3) * dt, int_par->nmon, int_par->level, int_par->mon_list);
		...
		if (n < int_par->nsteps)
		{
			gforce(momenta, 2 * r1 * dt, int_par->nmon, int_par->level, int_par->mon_list);
			lprintf("MD_INT", 20 + int_par->level * 10, "MD step (level %d): %d/%d\n", int_par->level, n, int_par->nsteps);
		}
		else
		{
